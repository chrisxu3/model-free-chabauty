...
function SimplifyModularFormBasis(M,F)
    /*  Input:
            ...
            F:  a sequence of Q-linearly indepdendent modular forms in M_{k,G} for some k>1
...
function FindRelationRationalBruteForce(M,h: d:=0, check:=false, wt:=0)
    /* Input:
                M:  a record M type "ModularCurveRec" (for example produced as output of CreateModularCurveRec) that 
                    corresponds to a modular curve X_G.  
                    We assume that X_G is an genus 0 curve isomorphic to P^1_Q and that Q(X_G)=Q(f), where f:=M`f is given by q-expansions at the cusps of M.
        ...
        Suppose that "check" is true and h is known to be a quotient of modular forms from M_{wt,G} where "wt" is an integer greater than 1.  Then the rational
...
function FindRelationEllipticBruteForce(M,h: d:=0, check:=false, wt:=0)
    /* Input:
                M:  a record M type "ModularCurveRec" (for example produced as output of CreateModularCurveRec) that 
                    corresponds to a modular curve X_G.  
        ...
        Suppose that "check" is true and h is known to be a quotient of modular forms from M_{wt,G} where "wt" is an integer greater than 1.  Then the function
...
function FindCoverOfModularCurve(M0,M,prec : simplify_serre_type:=true)   
    /*  Input:             
...
function AutomorphismOfModularForms(M,F,g : wt:=0)
    /*  Input:
            ...
            F   :linearly independent elements in M_{k,G} for some k>1

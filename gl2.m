/*
    Grothendieck should not have hated Deligne over his failure to prove the Weil conjectures via motives;
    rather, he should have hated Deligne for opting to define a level structure as an isomorphism E[N] -> (Z/NZ)^2 in
    Deligne-Rapoport, which has caused over half a century of confusion amongst those who play the glass bead game of modular curves.

    This module is basically "gl2.m" in RSZB, but we are working with the more natural definition of a level structure.
    Let us define what this means here. Pairs (E,\beta) where \beta : (Z/NZ)^2 \to E[N] gives a full set of sections, and
    E is a generalized elliptic curve. The elements of (Z/NZ)^2 are represented as column vectors. There is a left action of GL2
    on this space given by g.(E,\beta) = (E, \beta \circ {}^t g), and with slightly more work, one can show that X_G := G\X(N) are 
    given by pairs (E,[GgA_E]) where [GgA_E] = [Gg\sigmaA_E]
